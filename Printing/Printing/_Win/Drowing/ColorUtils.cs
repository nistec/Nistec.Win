    ...
    static public bool UsingCustomColor
      ...
      { 
        ...
        public static Color ColorDark(Color color)
        {
            ...
            int num = GreyScale(color);
            HLSRGB hlsrgb = new HLSRGB(color.R, color.G, color.B);
            if (num > 250)
            {
                hlsrgb.Luminance = ((float)WhiteDark.R) / 255f;
                return Color.FromArgb(color.A, hlsrgb.Color);
            }
            if (num < 0x40)
            {
                hlsrgb.Luminance = ((float)BlackDark.R) / 255f;
                return Color.FromArgb(color.A, hlsrgb.Color);
            }
            hlsrgb.DarkenColor(0.7f);
            return Color.FromArgb(color.A, hlsrgb.Color);
        }
        ...
        public static Color ColorDarkDark(Color color)
        {
            ...
            if (num > 250)
            {
                hlsrgb.Luminance = ((float)WhiteDarkDark.R) / 255f;
                return Color.FromArgb(color.A, hlsrgb.Color);
            }
            if (num < 0x40)
            {
                hlsrgb.Luminance = ((float)BlackDarkDark.R) / 255f;
                return Color.FromArgb(color.A, hlsrgb.Color);
            }
            hlsrgb.DarkenColor(0.5f);
            return Color.FromArgb(color.A, hlsrgb.Color);
        }
        ...
        public static Color ColorLight(Color color)
        {
            ...
            if (num > 250)
            {
                hlsrgb.Luminance = ((float)WhiteLight.R) / 255f;
                return Color.FromArgb(color.A, hlsrgb.Color);
            }
            if (num < 0x40)
            {
                hlsrgb.Luminance = ((float)BlackLight.R) / 255f;
                return Color.FromArgb(color.A, hlsrgb.Color);
            }
            hlsrgb.LightenColor(0.1f);
            return Color.FromArgb(color.A, hlsrgb.Color);
        }
        ...
        public static Color ColorLightLight(Color color)
        {
            ...
            if (num > 250)
            {
                hlsrgb.Luminance = ((float)WhiteLightLight.R) / 255f;
                return Color.FromArgb(color.A, hlsrgb.Color);
            }
            if (num < 0x40)
            {
                hlsrgb.Luminance = ((float)BlackLightLight.R) / 255f;
                return Color.FromArgb(color.A, hlsrgb.Color);
            }
            hlsrgb.LightenColor(0.5f);
            return Color.FromArgb(color.A, hlsrgb.Color);
        }

        public static void Draw3DBorder(Graphics gfx, Rectangle rect, Color color, bool thin, bool pressed)
        {
            Pen pen = new Pen(ColorDark(color), 1f);
            Pen pen2 = new Pen(ColorLight(color), 1f);
            if (thin)
            {
                if (pressed)
                {
                    gfx.DrawLine(pen, rect.Left, rect.Bottom - 2, rect.Left, rect.Top);
                    gfx.DrawLine(pen, rect.Left, rect.Top, rect.Right - 2, rect.Top);
                    gfx.DrawLine(pen2, rect.Right - 1, rect.Top, rect.Right - 1, rect.Bottom - 1);
                    gfx.DrawLine(pen2, rect.Right - 1, rect.Bottom - 1, rect.Left, rect.Bottom - 1);
                }
                else
                {
                    gfx.DrawLine(pen2, rect.Left, rect.Bottom - 2, rect.Left, rect.Top);
                    gfx.DrawLine(pen2, rect.Left, rect.Top, rect.Right - 1, rect.Top);
                    gfx.DrawLine(pen, (int)(rect.Right - 1), (int)(rect.Top + 1), (int)(rect.Right - 1), (int)(rect.Bottom - 1));
                    gfx.DrawLine(pen, rect.Right - 1, rect.Bottom - 1, rect.Left, rect.Bottom - 1);
                }
            }
            else
            {
                Pen pen3 = new Pen(ColorLightLight(color), 1f);
                Pen pen4 = new Pen(ColorDarkDark(color), 1f);
                if (pressed)
                {
                    gfx.DrawLine(pen4, rect.Left, rect.Bottom - 1, rect.Left, rect.Top);
                    gfx.DrawLine(pen4, rect.Left, rect.Top, rect.Right - 1, rect.Top);
                    gfx.DrawLine(pen3, (int)(rect.Right - 1), (int)(rect.Top + 1), (int)(rect.Right - 1), (int)(rect.Bottom - 1));
                    gfx.DrawLine(pen3, (int)(rect.Right - 1), (int)(rect.Bottom - 1), (int)(rect.Left + 1), (int)(rect.Bottom - 1));
                    gfx.DrawLine(pen, (int)(rect.Left + 1), (int)(rect.Bottom - 2), (int)(rect.Left + 1), (int)(rect.Top + 1));
                    gfx.DrawLine(pen, (int)(rect.Left + 1), (int)(rect.Top + 1), (int)(rect.Right - 2), (int)(rect.Top + 1));
                }
                else
                {
                    gfx.DrawLine(pen3, rect.Left, rect.Bottom - 1, rect.Left, rect.Top);
                    gfx.DrawLine(pen3, rect.Left, rect.Top, rect.Right - 1, rect.Top);
                    gfx.DrawLine(pen, (int)(rect.Right - 1), (int)(rect.Top + 1), (int)(rect.Right - 1), (int)(rect.Bottom - 1));
                    gfx.DrawLine(pen, (int)(rect.Right - 1), (int)(rect.Bottom - 1), (int)(rect.Left + 1), (int)(rect.Bottom - 1));
                    gfx.DrawLine(pen2, (int)(rect.Left + 1), (int)(rect.Bottom - 2), (int)(rect.Left + 1), (int)(rect.Top + 1));
                    gfx.DrawLine(pen2, (int)(rect.Left + 1), (int)(rect.Top + 1), (int)(rect.Right - 2), (int)(rect.Top + 1));
                }
                pen4.Dispose();
                pen3.Dispose();
            }
		...
		static public Color ColorFromPoint( Graphics g, int x, int y )
		{
			...
			uint colorref =(uint) WinAPI.GetPixel(hDC, x, y);
