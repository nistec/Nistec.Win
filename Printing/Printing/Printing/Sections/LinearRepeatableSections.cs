...
//	/// </remarks>
	public class LinearRepeatableSections : LinearSections
	{
            ...
            )
        {
            ...
            {
                divider.Print (printDocument, g, bounds);
                AdvancePointers (divider.Size, ref bounds, ref mySize);
            }

            // size first
            SectionSizeValues oneCall = SizePrintLine (printDocument, g, bounds, true, false);
            bool fits = oneCall.Fits;
            while (oneCall.Fits)
            {
                Bounds printBounds = bounds.GetBounds (oneCall.RequiredSize);
                SizePrintLine (printDocument, g, printBounds, false, true); // print
                ...
                oneCall = SizePrintLine (printDocument, g, bounds, true, false); // size
            }
