...
namespace Nistec.SyntaxEditor.UI
{
	...
	public class TextArea : UserControl
	{
        ...
        TextEditor/*TextEditorControl*/ motherTextEditorControl;
		...
		public TextEditor/*TextEditorControl*/ MotherTextEditorControl {
			get {
		...
		public int MaxVScrollValue {
			get {
                return (Document.GetVisibleLine(Document.TotalNumberOfLines - 1) + 1 + TextView.VisibleLineCount * 2 / 3) * Document.SyntaxProperties.Font.Height;
			}
		}
        ...
        {
			get {
                return motherTextEditorControl.SyntaxProperties;
			}
		}

        public TextArea(TextEditor/*TextEditorControl*/ motherTextEditorControl, TextAreaControl motherTextAreaControl)
		{
...
//			SetStyle(ControlStyles.UserPaint, true);
		...
		{
            if (!SyntaxProperties.ShowMatchingBracket)
            {
...
//				clipRectangle.Intersect(updateMargin.DrawingPosition);
			...
			if (this.SyntaxProperties.UseAntiAliasedFont) {
		...
		public void SimulateKeyPress(char ch)
		{
			...
			if (!HiddenMouseCursor && SyntaxProperties.HideMouseCursor) {
...
//				this.motherTextEditorControl.UpdateLines(currentLineNr, currentLineNr);
        ...
        internal bool handleSpase;
		protected override void OnKeyPress(System.Windows.Forms.KeyPressEventArgs e)
		{
            if (e.KeyChar == ' ')
            {
                e.Handled = handleSpase;
                if (handleSpase)
                {
                    handleSpase = false;
                    return;
				...
				try {
					lock (Document) {
						...
						if (SelectionManager.HasSomethingSelected && AutoClearSelection /*&& caretchanged*/) {
							if (Document.SyntaxProperties.DocumentSelectionMode == DocumentSelectionMode.Normal) {
...
//			++Caret.DesiredColumn;
		...
		public void ReplaceChar(char ch)
		{
			...
			if (Document.SyntaxProperties.DocumentSelectionMode == DocumentSelectionMode.Normal && SelectionManager.SelectionCollection.Count > 0) {
				Caret.Position = SelectionManager.SelectionCollection[0].StartPosition;
				SelectionManager.RemoveSelectedText();
			}
