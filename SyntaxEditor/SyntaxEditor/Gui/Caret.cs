...
namespace Nistec.SyntaxEditor.UI
{
	...
	public class Caret : System.IDisposable
	{
		...
		public Point ValidatePosition(Point pos)
		{
			...
			if (!textArea.SyntaxProperties.AllowCaretBeyondEOL) {
		...
		{
			if (textArea.MotherTextAreaControl.SyntaxProperties.LineViewerStyle == LineViewerStyle.FullRow && oldLine != line) {
			...
			try {
				...
				if (caretCreated) {
					...
					if (ime == null) {
						ime = new Ime(textArea.Handle, textArea.Document.SyntaxProperties.Font);
					} else {
						ime.Font = textArea.Document.SyntaxProperties.Font;
					}
