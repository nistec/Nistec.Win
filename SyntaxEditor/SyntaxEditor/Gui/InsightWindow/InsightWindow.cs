...
namespace Nistec.SyntaxEditor.UI.InsightWindow
{
	...
	{
		public InsightWindow(Form parentForm, TextEditor/*TextEditorControl*/ control, string fileName) : base(parentForm, control, fileName)
		{
		...
		public void HandleMouseWheel(MouseEventArgs e)
		{
			if (DataProvider != null && DataProvider.InsightDataCount > 0) {
				if (e.Delta > 0) {
					if (control.SyntaxProperties.MouseWheelScrollDown) {
						CurrentData = (CurrentData + 1) % DataProvider.InsightDataCount;
					} else {
						CurrentData = (CurrentData + DataProvider.InsightDataCount - 1) % DataProvider.InsightDataCount;
					}
