...
namespace Nistec.SyntaxEditor.UI
{
	...
	public class TextAreaControl : Panel
	{
        ...
        public TextAreaControl(TextEditor/*TextEditorControl*/ motherTextEditorControl)
		{
		...
		public void OptionsChanged()
		{
			...
			if (textArea.SyntaxProperties.ShowHorizontalRuler) {
				if (hRuler == null) {
		...
		{
			if ((Control.ModifierKeys & Keys.Control) != 0 && SyntaxProperties.MouseWheelTextZoom) {
				if (e.Delta > 0) {
				...
				if (System.Windows.Forms.SystemInformation.MouseWheelScrollLines > 0) {
					newValue = this.vScrollBar.Value - (SyntaxProperties.MouseWheelScrollDown ? 1 : -1) * Math.Sign(e.Delta) * System.Windows.Forms.SystemInformation.MouseWheelScrollLines * vScrollBar.SmallChange * multiplier;
				} else {
					newValue = this.vScrollBar.Value - (SyntaxProperties.MouseWheelScrollDown ? 1 : -1) * Math.Sign(e.Delta) * vScrollBar.LargeChange;
				}
