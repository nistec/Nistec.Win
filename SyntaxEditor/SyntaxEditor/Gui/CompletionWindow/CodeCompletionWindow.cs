...
namespace Nistec.SyntaxEditor.UI.CompletionWindow
{
	public class CodeCompletionWindow : AbstractCompletionWindow
	{
		...
		public static CodeCompletionWindow ShowCompletionWindow(Form parent, TextEditor/*TextEditorControl*/ control, string fileName, ICompletionDataProvider completionDataProvider, char firstChar)
		{
		...
		CodeCompletionWindow(ICompletionDataProvider completionDataProvider, Form parentForm, TextEditor/*TextEditorControl*/ control, string fileName) : base(parentForm, control, fileName)
		{
		...
		public void HandleMouseWheel(MouseEventArgs e)
		{
			...
			if (System.Windows.Forms.SystemInformation.MouseWheelScrollLines > 0) {
				newValue = this.vScrollBar.Value - (control.SyntaxProperties.MouseWheelScrollDown ? 1 : -1) * Math.Sign(e.Delta) * System.Windows.Forms.SystemInformation.MouseWheelScrollLines * vScrollBar.SmallChange * multiplier;
			} else {
				newValue = this.vScrollBar.Value - (control.SyntaxProperties.MouseWheelScrollDown ? 1 : -1) * Math.Sign(e.Delta) * vScrollBar.LargeChange;
			}
