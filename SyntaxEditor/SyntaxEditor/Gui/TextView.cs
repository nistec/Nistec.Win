...
namespace Nistec.SyntaxEditor.UI
{
	...
	public class TextView : AbstractMargin
	{
		...
		{
			this.fontHeight = SyntaxProperties.Font.Height;
...
#region Paint functions
		public override void Paint(Graphics g, Rectangle rect)
		{
			...
			if (fontHeight != SyntaxProperties.Font.Height) {
				OptionsChanged();
...
//					Console.WriteLine("logical line : " + currentLine);
		...
		void PaintDocumentLine(Graphics g, int lineNumber, Rectangle lineRectangle)
		{
			...
			if (lineNumber >= textArea.Document.TotalNumberOfLines) {
				...
				if (SyntaxProperties.ShowInvalidLines) {
					DrawInvalidLineMarker(g, lineRectangle.Left, lineRectangle.Top);
				}
				if (SyntaxProperties.ShowVerticalRuler) {
...
//				bgColorBrush.Dispose();
			...
			int column = 0;
			physicalColumn = 0;
			if (SyntaxProperties.EnableFolding) {
				while (true) {
			...
			if (lineNumber < textArea.Document.TotalNumberOfLines) {
				...
				if (SyntaxProperties.ShowEOLMarker) {
				...
				Brush fillBrush = selectionBeyondEOL && SyntaxProperties.AllowCaretBeyondEOL ? bgColorBrush : backgroundBrush; 
...
//			bgColorBrush.Dispose();
		...
		{
			return lineNumber == base.textArea.Caret.Line && textArea.MotherTextAreaControl.SyntaxProperties.LineViewerStyle == LineViewerStyle.FullRow;
		}
...
//							}
					...
					switch (currentWord.Type) {
						case TextWordType.Space:
							...
							if (SyntaxProperties.ShowSpaces) {
						...
						case TextWordType.Tab:
							...
							physicalColumn += SyntaxProperties.TabIndent;
							physicalColumn = (physicalColumn / SyntaxProperties.TabIndent) * SyntaxProperties.TabIndent;
...
#region Conversion Functions
		...
		{
			if (!charWitdh.ContainsKey(ch)) {
				charWitdh.Add(ch, g.MeasureString(ch.ToString(), SyntaxProperties.Font, 2000, measureStringFormat).Width);
			}
			return (float)charWitdh[ch];
		}
		...
		{
			int tabIndent = Document.SyntaxProperties.TabIndent;
			int column    = 0;
...
#region DrawHelper functions
		...
		{
			if (SyntaxProperties.VerticalRulerRow < textArea.VirtualTop.X) {
				return;
			}
			HighlightColor vRulerColor = textArea.Document.HighlightingStrategy.GetColorFor("VRuler");
			
			int xpos = (int)(drawingPosition.Left + GetWidth(g, ' ') * (SyntaxProperties.VerticalRulerRow - textArea.VirtualTop.X));
