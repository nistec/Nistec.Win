...
namespace Nistec.SyntaxEditor.Actions 
{
	...
	public class ConvertTabsToSpaces : AbstractSelectionFormatAction
	{
		protected override void Convert(IDocument document, int startOffset, int length)
		{
			...
			string spaces = new string(' ', document.SyntaxProperties.TabIndent);
			...
			for (int i = y2; i >= y1; --i) {
				...
				if(line.Length > 0) {
					...
					if(whiteSpace > 0) {
						...
						string newPrefix = newLine.Replace("\t",new string(' ', document.SyntaxProperties.TabIndent));
		...
		protected override void Convert(IDocument document, int y1, int y2) 
		{
			...
			for (int i = y2; i >= y1; --i) {
				...
				if(line.Length > 0) {
					...
					string newLine = TextUtilities.LeadingWhiteSpaceToTabs(document.GetText(line.Offset,line.Length), document.SyntaxProperties.TabIndent);
