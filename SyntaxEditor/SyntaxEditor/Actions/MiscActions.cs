...
namespace Nistec.SyntaxEditor.Actions
{
	public class Tab : AbstractEditAction
	{
		public static string GetIndentationString(IDocument document)
		{
			...
			if (document.SyntaxProperties.ConvertTabsToSpaces) {
				indent.Append(new String(' ', document.SyntaxProperties.TabIndent));
			} else {
				indent.Append('\t');
			}
...
//					string temp = document.GetText(line.Offset,line.Length);
		...
		public override void Execute(TextArea textArea)
		{
			if (textArea.SelectionManager.HasSomethingSelected) {
				...
				int tabIndent = textArea.Document.SyntaxProperties.TabIndent;
		...
		public override void Execute(TextArea textArea)
		{
			if (textArea.TextView.Highlight != null) {
				...
				if (p1 == textArea.Caret.Position) {
					if (textArea.Document.SyntaxProperties.BracketMatchingStyle == BracketMatchingStyle.After) {
