...
namespace Nistec.Charts
{
  ...
  /// </summary>
    [ToolboxBitmap(typeof(McPieChart), "Images.PieChart.bmp"), Designer("Nistec.Charts.Design.ControlDesignerBase")]
  public partial class McPieChart : Control
  {
    ...
    [Description("The leaning of the control, in radians.")]
    public float Leaning
    {
      get
      {
        return Style.Leaning;
      }
      set
      {
        Style.Leaning = value;
    ...
    [Category("Pie Chart")]
    [DefaultValue(10)]
    [Description("The depth of the pie, in pixels.")]
    public float Depth
    {
      get
      {
    ...
    [Browsable(true)]
    [Category("Pie Chart")]
    [DefaultValue(McPieChart.TextDisplayTypes.FitOnly)]
    ...
    //  [Category("Pie Chart")]
    //  [DefaultValue(0)]
    //  [Description("The internal Margin of Pie chart.")]
    ...
    /// <summary>
    /// Determines how transparent surfaces of pie slices are.  A SurfaceTransparency of 1 means the surfaces
    /// are completely opaque, while a SurfaceTransparency of 0 means the surfaces are completely transparent.
    /// </summary>
    //[Browsable(true)]
    //[Category("Pie Chart")]
    //[DefaultValue(1F)]
    ...
    //  get
    //  {
    //    return Style.SurfaceTransparency;
    //  }
    //  set
    //  {
    //    Style.SurfaceTransparency = value;
    //  }
    //}
    #endregion
    ...
    public Size GetChartSize(Padding padding)
    {
      ...
      int height = (int)(2 * (Radius * Style.HeightWidthRatio + maxOffset) + Style.VisualDepth + padding.Vertical);
    ...
    private float GetAutoSizeRadius(Rectangle bounds, Padding padding)
    {
      ...
      float height = (bounds.Height - padding.Vertical - Style.VisualDepth) / 2;
    ...
    private DrawingMetrics HitTest(PointF controlPoint)
    {
      ...
      PointF transPoint = new PointF(controlPoint.X - Width / 2, controlPoint.Y - (Height + Style.VisualDepth) / 2);
    ...
    private void Render(Graphics g, List<DrawingMetrics> drawingMetrics, Rectangle bounds, Padding padding)
    {
      ...
      g.TranslateTransform(bounds.Left + padding.Left + (bounds.Width - padding.Horizontal) / 2, bounds.Top + padding.Top + (bounds.Height - padding.Vertical + Style.VisualDepth) / 2);
    ...
    /// <param name="padding">The padding which defines the border of the image.</param>
    /// <returns></returns>
      public Bitmap GetImage(ImageFormat format, Size sizeInPixels, Padding padding)
      {
          List<DrawingMetrics> metrics = ConstructDrawingMetrics(new Rectangle(Point.Empty, sizeInPixels), padding);
          Bitmap bitmap = new Bitmap(sizeInPixels.Width, sizeInPixels.Height);
          using (Graphics g = Graphics.FromImage(bitmap))
          {
              // fill in the background
              g.FillRectangle(Brushes.White, 0, 0, sizeInPixels.Width, sizeInPixels.Height);
    ...
    protected override void OnPaint(PaintEventArgs pe)
    {
        ...
        Rectangle rect = new Rectangle(Margin.Left, Margin.Top, r.Width-(Margin.Left+Margin.Right), r.Height-(Margin.Top+Margin.Bottom));
      ...
      if (isStructureChanged && transactionRef == 0)
      {
        ...
        this.drawingMetrics = ConstructDrawingMetrics(rect/*this.ClientRectangle*/, this.Padding);
      }
      else if (isVisualChanged && recreateGraphics && transactionRef == 0)
      {
          RecreateGraphics(this.drawingMetrics, rect/*this.ClientRectangle*/);
      }
      ...
      if (!this.DesignMode && panelsCreated)
      {
          PaintPanels(pe.Graphics);
      }

      Render(pe.Graphics, this.drawingMetrics, rect/*this.ClientRectangle*/, Padding);
    }
    #endregion

    #region Panels 

    private List<Rectangle> PanelRect;
    private List<Rectangle> LabelRect;
    private List<Color> ColorList;
    private List<string> TextList;
    private bool panelsCreated = false;

    protected void PaintPanels(Graphics g)
      {
          int i = 0;
          foreach (Rectangle rect in PanelRect)
          {
              g.FillRectangle(new SolidBrush(ColorList[i]), rect);
              i++;
          }
          i = 0;
          foreach (Rectangle rect in LabelRect)
          {
              g.DrawString(TextList[i], Control.DefaultFont, Brushes.Black, (RectangleF)rect);
              i++;
          }
          g.DrawRectangles(Pens.Black, PanelRect.ToArray());
      ...
      private void CreateLists()
      {
          ...
          PanelRect = new List<Rectangle>(count);
          LabelRect = new List<Rectangle>(count);
          ColorList = new List<Color>(count);
          TextList = new List<string>(count);
          panelsCreated = true;
      }
      ...
      /// </summary>
      public void AddChartDescription()
      {
          CreateLists();
          int i = 0;
          foreach (PieChartItem p in Items)
          {
              AddRectangle(i,p.Color,p.GetPanelText());
              i++;
          }
      }
      /// <summary>
      ...
      /// </summary>
      public void ClearChartDescription()
      {
          ClearLists();
          this.Invalidate();
      }
      ...
      //public void AddChartDescription(string[] descriptions)
      //{
      ...
      //    int i = 0;
      //    int colorIndex = 0;
      //    int colorCount = ChartColors.Length;

      //    foreach (string s in descriptions)
      //    {
      //        AddRectangle(i, ChartColors[colorIndex], s);
      //        i++;
      //        colorIndex = colorIndex >= colorCount - 1 ? 0 : colorIndex + 1;
      //    }
      //}

      private void AddRectangle(int index, Color color, string text)
      {
          int top = 6 + index * 16;
          Rectangle r = this.ClientRectangle;
   
          Rectangle p = new Rectangle(r.X+ 3,r.Top+ top, 13, 13);
          PanelRect.Add(p);
          ColorList.Add(color);//ChartColors[colorIndex]);

          Rectangle l = new Rectangle(r.X+ 22,r.Top+ top, r.Width-22, 13);
          LabelRect.Add(l);
          TextList.Add(text);
