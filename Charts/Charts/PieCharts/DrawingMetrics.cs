    ...
    public partial class McPieChart : Control
  {
    ...
    internal class DrawingMetrics : IComparable<DrawingMetrics>
    {
      ...
      /// <param name="sweepAngle">The sweep angle of this pie slice, in radians.</param>
        public DrawingMetrics(McPieChart control, PieChartItem item, Rectangle drawingBounds, float startAngle, float sweepAngle)
      {
      ...
      {
        float brightnessFactor = Control.ItemStyle.EdgeBrightness;
        if (IsFocused)
          brightnessFactor = Control.FocusedItemStyle.EdgeBrightness;
      ...
      private void ConstructPaths()
      {
        ...
        topFace.AddPie(offset.Width - Control.Style.EllipseWidth, -Control.Style.VisualDepth + offset.Height - Control.Style.EllipseHeight, 2 * Control.Style.EllipseWidth, 2 * Control.Style.EllipseHeight, (float)(TransformedStartAngle * 180 / Math.PI), (float)(TransformedSweepAngle * 180 / Math.PI));
      ...
      private GraphicsPath CreateInteriorFace(float angle, out GraphicsPath edgePath)
      {
        ...
        points[0] = new PointF(offset.Width, offset.Height - Control.Style.VisualDepth);
        ...
        points[3] = new PointF(points[2].X, points[2].Y - Control.Style.VisualDepth);
      ...
      private void AddExteriorFigure(GraphicsPath path, GraphicsPath edgePath, PointF rightPoint, PointF leftPoint, float startAngle, float sweepAngle)
      {
        ...
        // calculate the exterior points with the depth of the slice taken into effect
        PointF rightPointThick = new PointF(rightPoint.X, rightPoint.Y - Control.Style.VisualDepth);
        PointF leftPointThick = new PointF(leftPoint.X, leftPoint.Y - Control.Style.VisualDepth);

        // draw the exterior figure
        path.StartFigure();
        path.AddLine(rightPoint, rightPointThick);
        path.AddArc(offset.Width - Control.Style.EllipseWidth, offset.Height - Control.Style.VisualDepth - Control.Style.EllipseHeight, 2 * Control.Style.EllipseWidth, 2 * Control.Style.EllipseHeight, (float)(startAngle * 180 / Math.PI), (float)(sweepAngle * 180 / Math.PI));
      ...
      public PointF GetTextPosition()
      {
        ...
        return new PointF((float)(OffsetSize.Width + Control.Style.EllipseWidth * 2 / 3 * Math.Cos(angle)), (float)(-Control.Style.VisualDepth + OffsetSize.Height + Control.Style.EllipseHeight * 2 / 3 * Math.Sin(angle)));
      }
      ...
      #region Methods
      /// <summary>
      /// Transforms the real angle of a pie slice into the angle that can be drawn at the given leaning.
      ...
      {
        float alpha = Control.ItemStyle.SurfaceTransparency;
        float brightnessFactor = Control.ItemStyle.SurfaceBrightness;
        if (Control.FocusedItem == this.Item)
        {
          alpha = Control.FocusedItemStyle.SurfaceTransparency;
          brightnessFactor = Control.FocusedItemStyle.SurfaceBrightness;
        }
