...
namespace Nistec.WinForms.Controls
{
    ...
    public class ImageButton :Control
    {
        ...
        {
            using (Brush sb = new SolidBrush((_mouseCapture && _mouseOver) ? Color.LightGray : _mouseOver ? Color.AliceBlue : Color.LightBlue))
            {
            ...
            if (_imageList != null)
            {
                ...
                {
                    // McButton is enabled, any caller supplied attributes to modify drawing?
                    if (null == _imageAttr)
                    {
                        // No, so use the simple DrawImage method
                        e.Graphics.DrawImage(_imageList.Images[_imageIndexEnabled], new Point(1,1));
                    ...
                    else
                    {
                        ...
                        // lower-left of the destination parallelogram. 
                        Point[] pts = new Point[3];
                        pts[0].X = 1;// (_mouseOver && _mouseCapture) ? 2 : 1;
                        pts[0].Y = 1;// (_mouseOver && _mouseCapture) ? 2 : 1;
                        pts[1].X = pts[0].X + image.Width;
                        pts[1].Y = pts[0].Y;
                        pts[2].X = pts[0].X;
                        pts[2].Y = pts[1].Y + image.Height;
                    ...
                    //}
                    bs = ButtonBorderStyle.Solid;
                    Color bc = Color.LightGray;// this.BackColor;
