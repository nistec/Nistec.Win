...
namespace mControl.WinCtl.Controls
{
	...
	public class CtlPanel : System.Windows.Forms.ScrollableControl,IStyleCtl,IPanel
	{
		...
		protected override void OnHandleCreated(EventArgs e)
		{
            ...
            //{
            //    mControl.Util.Net.netWinCtl.NetFram(this.Name,"Ctl"); 
            //}
		}
		...
		public CtlPanel(ControlLayout ctlLayout)
		{
		...
		internal CtlPanel(bool net):this()
		{
			m_netFram=net;
		}
...
//			m_Style.PropertyChanged+=new PropertyChangedEventHandler(m_Style_PropertyChanged);
		...
		private void DrawContainer(Graphics g,Rectangle bounds ,bool fillBack)
		{
			Rectangle rect=new Rectangle (bounds.X ,bounds.Y,bounds.Width-1 ,bounds.Height-1);

			//this.BackColor =CtlStyleLayout.Layout.FlatColorInternal;//FlatColorInternal;
			if(fillBack)
			{
				using(Brush b=CtlStyleLayout.GetBrushFlat())
				{
 		...
 		private void DrawPanelXP(Graphics g,Rectangle bounds)
		{
            ...
            else if (m_ControlLayout == ControlLayout.VistaLayout)
            {
                ...
                using (Brush sb = CtlStyleLayout.GetBrushVistaGradient(rect, gradiaentAngle))
                {
                    g.FillPath(sb, path);
                }
        ...
        protected void SerializeBackColor(Color value, bool force)
        {
            switch (m_ControlLayout)
            {
                case ControlLayout.Visual:
                case ControlLayout.XpLayout:
                    base.BackColor = CtlStyleLayout.Layout.ColorBrush1Internal;
                    break;
                default:
...
//		}
		...
		protected override void OnControlAdded(ControlEventArgs e)
		{
			...
			if( e.Control is IStyleCtl)
			{
				if(this.autoChildrenStyle && this.StylePainter!=null)
				{
					if(this.StylePainter.StylePlan != (Styles.None))
					{
						((IStyleCtl)e.Control).StylePainter=this.StylePainter;
						if(this.StylePainter.StylePlan == Styles.Custom)
							((IStyleCtl)e.Control).SetStyleLayout(this.CtlStyleLayout.Layout);
					}
				}
			}
		}
		...
		protected override void OnControlRemoved(ControlEventArgs e)
		{
			...
			if(this.autoChildrenStyle && this.StylePainter!=null)
			{
				((IStyleCtl)e.Control).StylePainter=null;
			}
...
//		}
		...
		protected virtual void SetChildrenStyle(bool clear)
		{
			foreach(Control c in this.Controls)
			{
				if( c is IStyleCtl)
				{
					((IStyleCtl)c).StylePainter=clear?null:this.StylePainter;
				}
			}
			this.Invalidate(true);
		}
...
//		}
		...
		public override string ToString()
		{
			...
			return (text1 + ", BorderStyle: " + CtlPanel.StringFromBorderStyle(this.m_BorderStyle));
		}
