...
namespace MControl.WinForms
{
	...
	public class McColumn: Component
	{
		...
		public McColumn(string name, string caption, int width,DataTypes dataType)
		{
			m_Name = name;
			m_Caption=caption;
			Width = width;
		...
		internal void ToggelSortOrder()
		{
			this.sortOrder=sortOrder==SortOrder.ASC?SortOrder.DESC:SortOrder.ASC;
		}
...
//		}
		...
		public Data.ExportColumnType[] GetExportColumns()
		{
			McColumn[] ctl=GetVisibleColumns();
			int i=0;
			
			Data.ExportColumnType[] col=new MControl.Data.ExportColumnType[ctl.Length];
			foreach(McColumn c in ctl)
			{
				col[i].Caption=c.Caption;
				col[i].ColumnOrdinal=c.ColumnOrdinal;
				i++;
			}
			return col;
		}
