...
namespace Nistec.WinForms.Design
{
	internal class TabPageDesigner : ParentControlDesigner
	{
        ...
        {
            return (parentDesigner.Component is McTabControl);
        }
		...
		public override void Initialize(IComponent component)
		{
			...
			this.tabPage = component as McTabPage;
		}
...
//				if (this.Control.Parent is McTabControl)
//				{
...
//				return rules1;
		...
		private void OnMoveLeft(object sender, EventArgs e)
		{
			...
			int indx = ((McTabControl)this.tabPage.Parent).TabPages.IndexOf(this.tabPage);
			...
			if (indx>0)// && num1 > 0)
			{
				try
				{
					...
					((McTabControl)this.tabPage.Parent).TabPages.MoveTo(indx-1,this.tabPage);
					...
					transaction1.Commit();
					((McTabControl)this.tabPage.Parent).SelectedTab=this.tabPage;
		...
		private void OnMoveRight(object sender, EventArgs e)
		{
			int indx = ((McTabControl)this.tabPage.Parent).TabPages.IndexOf(this.tabPage);
			int num1 = this.tabPage.Parent.Controls.GetChildIndex(this.tabPage);
			if (indx <(((McTabControl)this.tabPage.Parent).TabPages.Count-1))//&& num1 < (this.tabPage.Parent.Controls.Count - 1))
			{
				try
				{
					IDesignerHost host1 = (IDesignerHost) this.GetService(typeof(IDesignerHost));
		...
		private void CheckVerbStatus()
		{
			...
			if (this.moveRight != null)
				this.moveRight.Enabled=(indx <((McTabControl)this.tabPage.Parent).TabPages.Count-1);
		...
		protected override void Dispose(bool disposing)
		{
			if(disposing)
			{
				...
				{
					if(((McTabControl)this.tabPage.Parent).TabPages.Contains(this.tabPage))
					{
						((McTabControl)this.tabPage.Parent).TabPages.Remove(this.tabPage);
					}
				}
