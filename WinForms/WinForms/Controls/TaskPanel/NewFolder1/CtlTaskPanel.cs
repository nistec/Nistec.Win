...
namespace mControl.WinCtl.Controls
{
	...
	/// </summary>
	[ToolboxBitmap (typeof(CtlTaskPanel),"Toolbox.TaskPanel.bmp")]
	[ToolboxItem(false)]
	[Designer(typeof(Design.TaskPanelDesigner))]
	public class CtlTaskPanel : CtlPanel
	{
		...
		#region Constructors
		/// <summary>
		/// Initialises a new instance of <a cref="mContril.WinCtl.Controls.CtlTaskPanel">CtlTaskPanel</a>.
		/// </summary>
		public CtlTaskPanel() : base()
		{
			...
			controlSpace=8;
			image=null;
			lblImage=CtlTaskBar.imageExpand;
		...
		{
			this.components = new System.ComponentModel.Container();
			System.Resources.ResourceManager resources = new System.Resources.ResourceManager(typeof(CtlTaskPanel));
            ...
            // panelItems
            // 
            this.panelItems.BorderStyle = BorderStyle.None;
            this.panelItems.Cursor = System.Windows.Forms.Cursors.Default;
            this.panelItems.Dock = System.Windows.Forms.DockStyle.Fill;
            this.panelItems.Location = new System.Drawing.Point(0, 25);
            this.panelItems.Name = "panelItems";
            this.panelItems.Padding = new Padding(1);
            this.panelItems.TabIndex = 0;
            this.panelItems.Text = "";
            // 
			// CtlTaskPanel
			// 
            ...
            this.Controls.SetChildIndex(this.panelItems, 0);
            this.Padding = new Padding(1,0,1,1);

            this.items.Cleared += new mControl.Collections.CollectionClear(items_Cleared); 
		...
		private void items_Cleared()
		{
            ...
            //{
            //    if (c != this.labelTitle && c != this.panelItems)
            //    {
            //        this.Controls.Remove(c);
            //    }
            //}
            this.panelItems.Controls.Clear();  
		}
        ...
        internal void AddNewItem(CtlTreeView item)
        {
            items_Inserted(item);
        }
        internal void AddNewItem(CtlHelpLabel item)
        {
            items_Inserted(item);
        }
        internal void AddNewItem(CtlListBox item)
        {
            items_Inserted( item);
        }
		...
		{
	       if (value is mControl.WinCtl.Controls.LinkLabelItem)
            {
                this.panelItems.Controls.Add(value as mControl.WinCtl.Controls.LinkLabelItem);
                this.items[index].LinkClick += new LinkClickEventHandler(TaskPanel_LinkClick);
            }
        }
        private void items_Inserted(object value)
        {
            if (value is mControl.WinCtl.Controls.CtlTreeView)
            {
                AddTreeControl(value as CtlTreeView);
            }
            else if (value is mControl.WinCtl.Controls.CtlHelpLabel)
            {
                panelItems.Controls.Add((CtlHelpLabel)value);
                this.controlSpace = 1;
            }
        }

        //void CtlTaskPanel_SelectedIndexChanged(object sender, EventArgs e)
        //{
        //    throw new Exception("The method or operation is not implemented.");
        //}

        //void CtlTaskPanel_AfterSelect(object sender, TreeViewEventArgs e)
        //{
        //    throw new Exception("The method or operation is not implemented.");
        //}
		...
		{
			if(value is mControl.WinCtl.Controls.LinkLabelItem && index >0 && index < items.Count)
			{
				this.items[index].LinkClick -=new LinkClickEventHandler(TaskPanel_LinkClick);
                this.panelItems.Controls.Remove(value as mControl.WinCtl.Controls.LinkLabelItem); 
			}
        }
...
//		}
		...
		public Image Image
		{
			...
			set
			{
				...
				if(null != value)
				{
					...
					this.labelTitle.Height = this.image.Height + (2 * CtlTaskPanel.iconBorder);
        ...
        {
            this.owner.UpdateSlidePositions(this);
        }
...
//		}
		...
		private int GetButtomControls()
		{
			...
			foreach(Control c in this.panelItems.Controls )
			{
...
//					this.items[i].Top = this.items[i - 1].Bottom + this.ControlSpace;
		...
		/// <summary>
		/// Event handler for the <see cref="CtlTaskPanel.PanelStateChanged">PanelStateChanged</see> event.
		/// </summary>
        ...
        private void DrawPanelRounded(System.Windows.Forms.PaintEventArgs e)
        {
            ...
            if (Parent is IPanel)
                e.Graphics.Clear(((IStyleCtl)this.Parent).CtlStyleLayout.Layout.GetFlatColor(this.owner.FlatLayout));//FlatColorInternal);
            else
        ...
        private void DrawPanelRect(ControlLayout cl, System.Windows.Forms.PaintEventArgs e)
        {
            ...
            if (true == this.Enabled)
            {
                if (cl == ControlLayout.Visual)
                    ...
                    brush = CtlStyleLayout.GetBrushGradientDark(bounds, 90f, true);
        ...
        private void SetBoundOffset()
        {
            ...
            if (null != this.image)
            {
                offsetY = this.labelTitle.Height - CtlTaskPanel.minTitleHeight;
                if (offsetY < 0)
                {
                    offsetY = 0;
                }
        ...
        private void DrawLabelImage(Graphics g)
        {
            ...
            offsetX = CtlTaskPanel.iconBorder;
            if (null != this.image)
            {
                offsetX += this.image.Width + CtlTaskPanel.iconBorder;
                // Draws the title icon grayscale when the panel is disabled.
                RectangleF srcRectF = this.image.GetBounds(ref graphicsUnit);
                Rectangle destRect = new Rectangle(CtlTaskPanel.iconBorder,
                    CtlTaskPanel.iconBorder, this.image.Width, this.image.Height);
                if (true == this.Enabled)
                {
        ...
        private void DrawLabelString(ControlLayout cl, Graphics g)
        {
            ...
            using (Brush textBrush = cl == ControlLayout.System ? CtlStyleLayout.GetBrushCaption() : CtlStyleLayout.GetBrushContent())//GetBrushText())
            {
                ...
                float top = (float)offsetY + (float)CtlTaskPanel.expandBorder;
                float width = (float)this.labelTitle.Width - left - this.imageList.ImageSize.Width -
                    CtlTaskPanel.expandBorder;
                float height = (float)CtlTaskPanel.minTitleHeight - (2f * (float)CtlTaskPanel.expandBorder);
        ...
        private void DrawLabelImageState(Graphics g, Rectangle bounds)
        {
            if (!this.hookPanel)
            {
                ...
                int xPos = bounds.Right - this.imageList.ImageSize.Width - CtlTaskPanel.expandBorder;
                int yPos = bounds.Top + CtlTaskPanel.expandBorder;
        ...
        private mControl.WinCtl.Controls.CtlTreeView treeView;

        private void AddTreeControl(CtlTreeView ctl)
        {
            if (this.Controls.ContainsKey(ctl.Name))//"treeView"))
                return;
            this.SuspendLayout();
            this.treeView = ctl;// new mControl.WinCtl.Controls.CtlTreeView();
            // 
            // treeView
            // 
        ...
        public CtlTreeView TreeControl
        {
            get { return this.treeView; }
        }

        private void RemoveTreeControl()//CtlTreeView treeView)
        {
