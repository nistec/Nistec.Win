...
namespace Nistec.WinForms
{
    ...
    [ToolboxBitmap(typeof(McComboBox), "Toolbox.ComboBox.bmp")]
    public class McComboBox : McComboBase, IComboList, IDropDown
    {
        ...
        internal protected Rectangle TextRectInternal = Rectangle.Empty;
        ...
        {
            //if (!gridBounded)// if (this.DropDownStyle != ComboBoxStyle.DropDownList)
            //{
                base.OnValidating(e);
                IsValidating();
            //}
        ...
        private bool Validate_Item()
        {
            if (this.AcceptItems)
            {
                int index = ((McListItems)this.InternalList).Items.IndexOf(this.Text);
                if (index > -1)
                {
                    ((McListItems)this.InternalList).SelectedIndex = index;
                }
                return (index < 0);
            }
            return false;
        ...
        internal virtual void UpdateSelection(int index, bool checkItems)
        {
            UpdateSelection(index, checkItems, true);
        }

        internal virtual void UpdateSelection(int index, bool checkItems, bool updateText)
        {
            bool shouldRaiseEvent = gridBounded || SelectedIndex != index;
            //if (gridBounded || SelectedIndex != value)
            //{
            if (index == -1)
                {
                ...
                if (checkItems)
                {
                    object obj = InternalList.Items[index];
                    if (obj == null)
                    {
                        return;
                    }
        ...
        {
            return ((McListItems)this.InternalList).FindString(s);
        }

        public int FindString(string s, int startIndex)
        {
            return ((McListItems)this.InternalList).FindString(s, startIndex);
        }
        ...
        protected virtual object GetItemValue(object item)
        {
            if (item is DataRowView)
            {
                return ((DataRowView)item).Row[ValueMember.Substring(ValueMember.IndexOf(".") + 1)];
            }
            else //if (item is string)
            {
                return item;
        ...
        private void RefreshItems()
        {
            ...
            McListItems.ObjectCollection collection1 = Items;
            this.Items.Clear();
        ...
        private void SetDataConnection(object newDataSource, string newDisplayMember, bool force)
        {
            ...
            if ((force || flag1) || flag2)
            {
                ...
                //{
                //    Nistec.Util.BindControl bind = new BindControl();
        ...
        protected virtual void OnDrawItem(DrawItemEventArgs e)
        {
            ...
            else if (this.DrawMode == DrawMode.OwnerDrawFixed || this.DrawMode == DrawMode.OwnerDrawVariable)
            {
                ...
                if ((e.Index != -1) && (this.Items.Count > 0))
                {
                    ...
                    this.LayoutManager.DrawItem(graphics1, rectangle1, this, e.State, this.GetItemText(this.Items[e.Index]));
                }
            }
        }
        ...
        protected virtual void OnDrawItemInternal(DrawItemEventArgs e)
        {
            ...
            if ((e.Index != -1) && (this.Items.Count > 0))
            {
                ...
                LayoutManager.DrawItem(graphics1, rectangle1, this, e.State, this.GetItemText(this.Items[e.Index]));
            }
        }
        #endregion
        ...
        {
            base.EndHook();
            int selectedIndex = m_ComboPopUp.SelectedIndex;
            if (selectedIndex != m_SelectedIndex)
            {
                UpdateSelection(selectedIndex, false, true);
            }
            //if (!IgnorSelection)
            //{
        ...
        {
            get { return ((McListItems)this.InternalList).DataSource; }
            set
            {
        ...
        public virtual object SelectedValue
        {
            ...
            set
            {
                ...
                if (m_DataManager != null && this.InternalList != null)
                {
                    ...
                    if (gridBounded)
                    {
                        index = InternalList.SelectedIndex;
                    }
                    //else if (value.ToString() == Text)
                    //{
                    //    index = InternalList.SelectedIndex;
                    //}
                    else //base.DataBindings.Count>0
                    {
                        PropertyDescriptorCollection collection1 = m_DataManager.GetItemProperties();
                        PropertyDescriptor descriptor1 = collection1.Find(m_ValueMember, true);
                        index = DataManagerFind(descriptor1, value, true);
                    }
                    bool shouldRaiseEvent = index != SelectedIndex ;
        ...
        public ComboBoxStyle DropDownStyle
        {
            ...
            set
            {
                if (m_DropDownStyle != value)
                {
                    ...
                    //McComboBox.PropStyle=(int) value;
                    if (base.IsHandleCreated)
                    {
                        base.RecreateHandle();
                    }
        ...
        public class ComboPopUp : Nistec.WinForms.Controls.McPopUpBase
        {
            ...
            private void InitializeList()
            {
                ...
                // internalList
                // 
                //this.internalList.ownerMc=this;
            ...
            internal void ListSettingChanged()
            {
                ...
                internalList.ForeColor = Mc.ForeColor;
                internalList.RightToLeft = Mc.RightToLeft;

                this.BackColor = Mc.BackColor;
                Mc.shouldResetList = false;
            }
            ...
            public ComboPopUp(McComboBox parent)
                : base(parent)
            {
                Mc = parent;
            ...
            public override void ShowPopUp(IntPtr hwnd)
            {
                ...
                //this.Height = (Mc.ItemHeight * visibleItems) + heightAdd;
                int height = (Mc.ItemHeight * visibleItems) + heightAdd;
                this.Height = 0;
                

                selectedIndex = internalList.FindStringExact(selectedText);
                base.ShowPopUp(hwnd);
                ...
                if (Mc.DropDownWidth == 0)
                    ...
                    this.Width = Mc.DropDownWidth < Mc.Width ? Mc.Width : Mc.DropDownWidth;
                ...
                if (selectedIndex > -1)
                {
                    internalList.SelectedIndex = selectedIndex;
            ...
            private void internalList_KeyUp(object sender, System.Windows.Forms.KeyEventArgs e)
            {
                ...
                else if (Mc.allowAutoComplete)
                {
                    this.selectedItem = internalList.SelectedItem;
                    Mc.UpdateSelection(selectedItem);
                    //Mc.UpdateText();
                }
            }
            ...
            {
                this.selectedItem = internalList.SelectedItem;
                this.selectedIndex = internalList.SelectedIndex;
                this.Close();
            }

            void internalList_MouseWheel(object sender, MouseEventArgs e)
            {
                if (Mc.ReadOnly)// && this.DropDownStyle == ComboBoxStyle.Simple)
                {
                    return;
                }
            ...
            protected override void OnClosed(System.EventArgs e)
            {
                ...
                //Mc.internalList.KeyUp -= new System.Windows.Forms.KeyEventHandler(this.internalList_KeyUp);
                //Mc.internalList.SelectionChanged-=new EventHandler(internalList_SelectionChanged);
                //Mc.internalList.Visible=false; 

                //this.Controls.Remove(Mc.internalList);
                this.Hide();
                base.OnClosed(e);
            }
        ...
        ////private McListItems.ObjectCollection itemsCollection;
        ////private McListItems.ObjectCollection ObjectCollection;
        ...
        //        return textArray;
        //    }
        //    if (!(collection is McListItems.ObjectCollection))
        //    {
        //        return new string[0];
        //    }
