...
public class ComboBox : ListControl
{
    ...
    internal virtual Rectangle base_ApplyBoundsConstraints(int suggestedX, int suggestedY, int proposedWidth, int proposedHeight)
    {
    ...
    internal virtual IntPtr base_InitializeDCForWmCtlColor(IntPtr dc, int msg)
    {
    ...
    internal virtual void base_RecreateHandleCore()
    {
        lock (this)
        {
            if (this.IsHandleCreated)
            {
                ...
                HandleRef hWnd = new HandleRef(this, UnsafeNativeMethods.GetParent(new HandleRef(this, this.Handle)));
                try
                {
                ...
                {
                    if (((hWnd.Handle != IntPtr.Zero) && ((FromHandleInternal(hWnd.Handle) == null) || (this.parent == null))) && UnsafeNativeMethods.IsWindow(hWnd))
                    {
                        UnsafeNativeMethods.SetParent(new HandleRef(this, this.Handle), hWnd);
                    }
                }
                if (containsFocus)
                {
    ...
    internal virtual bool base_ShouldSerializeText()
    {
    ...
    internal Rectangle ApplyBoundsConstraints(int suggestedX, int suggestedY, int proposedWidth, int proposedHeight)
    {
        ...
        return base_ApplyBoundsConstraints(suggestedX, suggestedY, proposedWidth, proposedHeight);
    }
    ...
    internal bool ShouldSerializeText()
    {
        ...
        {
            return base_ShouldSerializeText();
        }
        return false;
    }
