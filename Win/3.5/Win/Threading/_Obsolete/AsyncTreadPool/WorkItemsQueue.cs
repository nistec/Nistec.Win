...
namespace MControl.Threading
{
	...
	/// </summary>
	internal class WorkItemsQueue : IDisposable
	{
		...
		/// </summary>
		private WaiterEntry _headWaiterEntry = null;
		...
		/// objects each thread stores its own WaiterEntry.
		private WaiterEntry GetThreadWaiterEntry()
		{
			WaiterEntry waiterEntry = Thread.GetData(_slot) as WaiterEntry;
			if (null == waiterEntry)
			{
				waiterEntry = new WaiterEntry();
		...
		private void PushWaiter(WaiterEntry newWaiterEntry)
		{
			...
			if (null == _headWaiterEntry)
			{
				_headWaiterEntry = newWaiterEntry;
			}
			// If the stack is not empty then put newWaiterEntry as the new head 
			// of the stack.
			else
			{
				_headWaiterEntry._prevWaiterEntry = newWaiterEntry;
				newWaiterEntry._nextWaiterEntry = _headWaiterEntry;
				_headWaiterEntry = newWaiterEntry;
			}
		...
		private WaiterEntry PopWaiter()
		{
			...
			WaiterEntry orgHeadWaiterEntry = _headWaiterEntry;

			// Store the new stack head
			WaiterEntry newHeadWaiterEntry = _headWaiterEntry._nextWaiterEntry;

			// Update the old stack head list links and decrement the number
			// waiters.
			RemoveWaiter(orgHeadWaiterEntry, true);

			// Update the new stack head
			_headWaiterEntry = newHeadWaiterEntry;

			// Return the old stack head
			return orgHeadWaiterEntry;
		}
		...
		private class WaiterEntry : IDisposable
		{
			#region Member variables
