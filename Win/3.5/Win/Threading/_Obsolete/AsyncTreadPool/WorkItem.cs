...
namespace MControl.Threading
{
	...
	internal class WorkItem : IDisposable
	{
		...
		/// </summary>
		private ThreadContextCaller _callerContext;
			...
			PostExecuteCall callToPostExecute)
		{
			if (useCallerContext)
			{
				_callerContext = ThreadContextCaller.Capture();
			}

			_postExecuteWorkItemCallback = postExecuteWorkItemCallback;
			_postExecuteCalling = callToPostExecute;
			_callback = callback;
			_state = state;
			...
			lock(this)
			{
				if (WorkItemState.Canceled == _workItemState)
				{
					...
					if ((_postExecuteWorkItemCallback != null) &&
                        ((_postExecuteCalling & PostExecuteCall.WhenWorkItemCanceled) == PostExecuteCall.WhenWorkItemCanceled))
					{
						result = true;
					}
		...
		public void Execute()
		{
            PostExecuteCall currentCallToPostExecute = 0;

            // Execute the work item if we are in the correct state
            switch(_workItemState)
            {
                case WorkItemState.InProgress:
                    currentCallToPostExecute |= PostExecuteCall.WhenWorkItemNotCanceled;
                    ExecuteWorkItem();
                    break;
                case WorkItemState.Canceled:
                    currentCallToPostExecute |= PostExecuteCall.WhenWorkItemCanceled;
                    break;
                default:
                    Debug.Assert(false);
			...
			if ((currentCallToPostExecute & _postExecuteCalling) != 0)
			{
				PostExecute();
			}
		}
        ...
        {
            ThreadContextCaller ctc = null;
            if (null != _callerContext)
            {
                ctc = ThreadContextCaller.Capture();
                ThreadContextCaller.Apply(_callerContext);
            }
