...
namespace mControl.Threading
{
    ...
    public class AsyncCommand : IDisposable//:Component
    {
        ...
        private mControl.Data.IDBCmd dbcmd;
        ...
        public AsyncCommand(string cnn,DBProvider provider)
        {
            ...
            dbcmd = mControl.Data.DBUtil.Create(cnn,provider);
         ...
         public AsyncCommand(IDbConnection cnn, AsyncProgressLevel level)
        {
            ...
            _MessageLevel = level;
            connection = cnn;
            dbcmd = mControl.Data.DBUtil.Create(cnn);
            InitializeComponent();
        }
        ...
        private DataTable RunAsyncCall(IDbCommand dbCommand, string tableName)
        {
            try
            {
                Thread th = Thread.CurrentThread;
                DataTable dt = null;            
                dt =dbcmd.Execute_DataTable(tableName,dbCommand.CommandText);
                return dt;
            }
            catch (Exception ex)
            {
