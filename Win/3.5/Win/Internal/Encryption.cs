...
namespace MControl
{
    ...
    internal class Encryption
    {
        ...
        public static bool TryFromBase64String(string text, bool useDecode, out string result)
        {
            ...
            try
            {
                ...
                if (!MControl.Regx.RegexValidateIgnoreCase("[A-Za-z0-9+/=]", text))
                {
                    //result = text;
                    return false;
                }
        ...
        public static string Decrypt(string text, string strKey, bool useDecode=false)
        {
            ...
            try
            {
                ...
                //if (!TryFromBase64String(text, useDecode, out value))
                //{
                //    return value;
                //}
        ...
        public static bool Delock(string text)
        {
            ...
            try
            {
                //string prefix = MControl.Runtime.AssemblyResolver.GetAssemblyKey(type);
                byte[] key = { 65, 68, 53, 52, 54, 55, 84, 72 };
                DESCryptoServiceProvider des = new DESCryptoServiceProvider();
                Byte[] byteArray = Convert.FromBase64String(text);
                MemoryStream memoryStream = new MemoryStream();
        ...
        //{
        //    string hash = Convert.ToBase64String(HashPassword(password));
        //    return hash;
        //}
    ...
    //public static string EncryptArgs(string data, EncyptionType encType)
    //{
    //    switch (encType)
    //    {
    //        case EncyptionType.ENU64:
    //            return "ENU64" + Encrypt(data, true);
    //        case EncyptionType.ENHEX:
    //            return "ENHEX" + EncryptToHexString(data);
    //        case EncyptionType.ENB64:
    //        default:
    //            return "ENB64" + Encrypt(data, true);
    //    }
    //}

    //public static string DecryptArgs(string data, EncyptionType encType)
    //{

    //    if (data.StartsWith("ENHEX"))
    //    {
    //        return DecryptFromHexString(data.Remove(0, 5));
    //    }
    //    switch (encType)
    //    {
    //        case EncyptionType.ENU64:
    //            return Decrypt(data.Remove(0, 5), true);
    //        case EncyptionType.ENHEX:
    //            return DecryptFromHexString(data.Remove(0, 5));
    //        case EncyptionType.ENB64:
    //            return Decrypt(data.Remove(0, 5), false);
    //        default:
    //            if (data.Contains("%"))
