	...
	public sealed class Validator 
	{
		...
		public static bool CanConvert(string text, BaseDataType type)
		{
			if(type==BaseDataType.Currency)
				return WinRegx.IsCurrency(text);
            else
...
//		}
		...
		public static bool ValidateRegex(string Text,string WinRegx,ref string message)
		{
			if (Text.Length ==0 || WinRegx.Length ==0)
			{
			...
			{
				Regex regularExpr = new Regex(WinRegx);
				bool ok= regularExpr.IsMatch(Text);
				if(!ok)
					message= RM.GetString(RM.InvalidRegx);						
				return ok;
