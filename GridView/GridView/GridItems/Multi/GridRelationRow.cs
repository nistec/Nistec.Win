...
namespace mControl.GridStyle
{
	...
	internal class GridRelationshipRow : GridRow
	{
		...
		{
			return new GridRelationshipRow.GridRelationshipRowAccessibleObject(this);
		}
		...
		{
			Rectangle rectangle1 = base.GetCellBounds(col);
			rectangle1.Height = base.Height - 1;
			return rectangle1;
		}
		...
		public override bool OnKeyPress(Keys keyData)
		{
			...
			switch ((keyData & Keys.KeyCode))
			{
				...
				case Keys.Return:
					...
					{
						base.Grid.NavigateTo((string) base.dgTable.RelationsList[this.FocusedRelation], this, true);
						this.FocusedRelation = -1;
						return true;
					}
					return false;
		...
		public override bool OnMouseMove(int x, int y, Rectangle rowHeaders, bool alignToRight)
		{
			...
			base.Grid.Cursor = Cursors.Default;
		...
		public override void PaintHeader(Graphics g, Rectangle bounds, bool alignToRight, bool isDirty)
		{
			...
			{
				this.PaintHeaderInside(g, rectangle1, base.Grid.HeaderBackBrush, alignToRight, isDirty);
			}
			else
			{
				this.PaintHeaderInside(g, rectangle1, base.dgTable.dataGrid.HeaderBackBrush, alignToRight, isDirty);
			}
		}

		public void PaintHeaderInside(Graphics g, Rectangle bounds, Brush backBr, bool alignToRight)
		{
			this.PaintHeaderInside(g, bounds, backBr, alignToRight, false);
		}
		...
		public void PaintHeaderInside(Graphics g, Rectangle bounds, Brush backBr, bool alignToRight, bool isDirty)
		{
			...
			base.PaintHeader(g, rectangle1, alignToRight, isDirty);
		...
		{
			if (((base.dgTable.RelationsList.Count == 0) || (base.dgTable.Grid == null)) || !base.dgTable.Grid.AllowNavigation)
			{
				return false;
			}
			if (!this.expanded)
			...
			Rectangle rectangle2 = this.GetRelationshipRect();
			rectangle2.X += rowHeaders.X + base.dgTable.dataGrid.RowHeaderWidth;
			rectangle2.X = this.MirrorRelationshipRectangle(rectangle2, rowHeaders, alignToRight);
			this.InvalidateRowRect(rectangle2);
			this.FocusedRelation++;
			return true;
		}
		...
		// Nested Types
		[ComVisible(true)]
			protected class GridRelationshipAccessibleObject : AccessibleObject
		{
			// Methods
			public GridRelationshipAccessibleObject(GridRelationshipRow owner, int relationship)
			{
				this.owner = null;
				this.owner = owner;
			...
			public override void DoDefaultAction()
			{
				...
				this.Grid.NavigateTo((string) this.owner.dgTable.RelationsList[this.relationship], this.owner, true);
			}
			...
			public override Rectangle Bounds
			{
				get
				{
					Rectangle rectangle1 = this.Grid.GetRowBounds(this.owner);
					Rectangle rectangle2 = this.owner.Expanded ? this.owner.GetRelationshipRectWithMirroring() : Rectangle.Empty;
					rectangle2.Y += this.owner.dgTable.RelationshipHeight * this.relationship;
					rectangle2.Height = this.owner.Expanded ? this.owner.dgTable.RelationshipHeight : 0;
					if (!this.owner.Expanded)
					{
						rectangle2.X += rectangle1.X;
					}
					rectangle2.Y += rectangle1.Y;
					return this.owner.Grid.RectangleToScreen(rectangle2);
				}
			}
			protected Grid Grid
			{
			...
			protected GridRelationshipRow Owner
			{
				get
				{
					return this.owner;
			...
			public override AccessibleStates State
			{
				get
				{
					AccessibleStates states1 = AccessibleStates.Linked | AccessibleStates.Selectable | AccessibleStates.Focusable;
					if (!this.owner.Expanded)
					{
						states1 |= AccessibleStates.Invisible;
					}
					if (this.Grid.Focused && (this.Owner.dgTable.FocusedRelation == this.relationship))
					{
			...
			// Fields
			private GridRelationshipRow owner;
			private int relationship;
		}

		[ComVisible(true)]
		protected class GridRelationshipRowAccessibleObject : GridRow.GridRowAccessibleObject
		{
			// Methods
			public GridRelationshipRowAccessibleObject(GridRow owner) : base(owner)
		{
		}
			...
			protected override void AddChildAccessibleObjects(IList children)
			{
				...
				GridRelationshipRow row1 = (GridRelationshipRow) base.Owner;
				if (row1.dgTable.RelationsList != null)
				{
					for (int num1 = 0; num1 < row1.dgTable.RelationsList.Count; num1++)
					{
						children.Add(new GridRelationshipRow.GridRelationshipAccessibleObject(row1, num1));
					}
				}
			}
			...
			public override void DoDefaultAction()
			{
				...
				{
					((GridRelationshipRow) base.Owner).Expanded = !((GridRelationshipRow) base.Owner).Expanded;
				}
			}
			...
			{
				GridRelationshipRow row1 = (GridRelationshipRow) base.Owner;
				int num1 = row1.dgTable.FocusedRelation;
				if (num1 == -1)
				{
					return base.GetFocused();
				}
				return this.GetChild((this.GetChildCount() - row1.dgTable.RelationsList.Count) + num1);
			}
			...
			public override string DefaultAction
			{
				get
				{
					...
					{
						return "AccDGCollapse";
					}
					return "AccDGExpand";
				}
			}
 
			private GridRelationshipRow Row
			{
				get
				{
					return (GridRelationshipRow) base.Owner;
				}
			}
			public override AccessibleStates State
			{
				get
				{
					AccessibleStates states1 = base.State;
					if (this.Row.dgTable.RelationsList.Count <= 0)
					{
						return states1;
					}
					if (((GridRelationshipRow) base.Owner).Expanded)
					{
