...
namespace mControl.GridView.Develop
{
    ...
    internal static class DesignerUtils
    {
        ...
        public static void DrawGrabHandle(Graphics graphics, Rectangle bounds, bool isPrimary, Glyph glyph)
        {
            ...
            try
            {
                IntPtr handle = SafeNativeMethods.SelectObject(new HandleRef(glyph, hdc), new HandleRef(glyph, isPrimary ? grabHandleFillBrushPrimary : grabHandleFillBrush));
                IntPtr ptr3 = SafeNativeMethods.SelectObject(new HandleRef(glyph, hdc), new HandleRef(glyph, isPrimary ? grabHandlePenPrimary : grabHandlePen));
                ...
                SafeNativeMethods.SelectObject(new HandleRef(glyph, hdc), new HandleRef(glyph, ptr3));
            }
            finally
            {
        ...
        public static void DrawLockedHandle(Graphics graphics, Rectangle bounds, bool isPrimary, Glyph glyph)
        {
            ...
            try
            {
                IntPtr handle = SafeNativeMethods.SelectObject(new HandleRef(glyph, hdc), new HandleRef(glyph, grabHandlePenPrimary));
                IntPtr ptr3 = SafeNativeMethods.SelectObject(new HandleRef(glyph, hdc), new HandleRef(glyph, grabHandleFillBrushPrimary));
                SafeNativeMethods.RoundRect(new HandleRef(glyph, hdc), bounds.Left + LOCKHANDLEUPPER_OFFSET, bounds.Top, (bounds.Left + LOCKHANDLEUPPER_OFFSET) + LOCKHANDLESIZE_UPPER, bounds.Top + LOCKHANDLESIZE_UPPER, 2, 2);
        ...
        public static void DrawNoResizeHandle(Graphics graphics, Rectangle bounds, bool isPrimary, Glyph glyph)
        {
            ...
            {
                IntPtr handle = SafeNativeMethods.SelectObject(new HandleRef(glyph, hdc), new HandleRef(glyph, isPrimary ? grabHandleFillBrushPrimary : grabHandleFillBrush));
                IntPtr ptr3 = SafeNativeMethods.SelectObject(new HandleRef(glyph, hdc), new HandleRef(glyph, grabHandlePenPrimary));
        ...
        public static void GenerateSnapShotWithBitBlt(Control control, ref Image image)
        {
            ...
            using (Graphics graphics = Graphics.FromImage(image))
            {
                ...
                IntPtr hdc = graphics.GetHdc();
                SafeNativeMethods.BitBlt(hdc, 0, 0, image.Width, image.Height, dC, 0, 0, 0xcc0020);
        ...
        public static bool GenerateSnapShotWithWM_PRINT(Control control, ref Image image)
        {
            ...
            using (Graphics graphics2 = Graphics.FromImage(image))
            {
                ...
                NativeMethods.SendMessage(handle, 0x317, hdc, (IntPtr)30);
