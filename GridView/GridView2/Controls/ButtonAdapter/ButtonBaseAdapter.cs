namespace System.Windows.Forms.ButtonInternal
{
    ...
    internal abstract class ButtonBaseAdapter
    {
        ...
        internal virtual LayoutOptions CommonLayout()
        {
            ...
            options.client = LayoutUtils.DeflateRect(this.Control.ClientRectangle, this.Control.Padding);
            options.padding = this.Control.Padding;
            options.growBorderBy1PxWhenDefault = true;
            options.isDefault = this.Control.IsDefault;
            options.borderSize = 2;
            options.paddingSize = 0;
            options.maxFocus = true;
            options.focusOddEvenFixup = false;
            options.font = this.Control.Font;
            options.text = this.Control.Text;
            options.imageSize = (this.Control.Image == null) ? Size.Empty : this.Control.Image.Size;
            options.checkSize = 0;
            options.checkPaddingSize = 0;
            options.checkAlign = ContentAlignment.TopLeft;
            options.imageAlign = this.Control.ImageAlign;
            options.textAlign = this.Control.TextAlign;
            ...
            options.textImageRelation = this.Control.TextImageRelation;
            options.useCompatibleTextRendering = this.Control.UseCompatibleTextRendering;
        ...
        {
            LayoutOptions options = new LayoutOptions();
            options.client = LayoutUtils.DeflateRect(clientRectangle, padding);
            ...
            options.font = font;
            options.text = text;
            options.imageSize = Size.Empty;
            options.checkSize = 0;
            options.checkPaddingSize = 0;
            options.checkAlign = ContentAlignment.TopLeft;
            options.imageAlign = ContentAlignment.MiddleCenter;
            options.textAlign = textAlign;
            options.hintTextUp = false;
            options.shadowedText = !enabled;
            options.layoutRTL = RightToLeft.Yes == rtl;
            options.textImageRelation = TextImageRelation.Overlay;
            options.useCompatibleTextRendering = false;
            return options;
        }

        private ColorOptions CommonRender(Graphics g)
        {
            ColorOptions options = new ColorOptions(g, this.Control.ForeColor, this.Control.BackColor);
            options.enabled = this.Control.Enabled;
            return options;
        }

        private static ColorOptions CommonRender(Graphics g, Color foreColor, Color backColor, bool enabled)
        {
            ColorOptions options = new ColorOptions(g, foreColor, backColor);
        ...
        private void Draw3DBorderNormal(Graphics g, ref Rectangle bounds, ColorData colors)
        {
            using (WindowsGraphics graphics = WindowsGraphics.FromGraphics(g))
            {
                ...
                Point point3 = new Point(bounds.X, (bounds.Y + bounds.Height) - 1);
                Point point4 = new Point((bounds.X + bounds.Width) - 1, (bounds.Y + bounds.Height) - 1);
                WindowsPen pen = new WindowsPen(graphics.DeviceContext, colors.buttonShadowDark);
                try
                {
                    graphics.DrawLine(pen, point, point2);
                    graphics.DrawLine(pen, point2, point3);
                ...
                pen = new WindowsPen(graphics.DeviceContext, colors.highlight);
                try
                {
                    point.Offset(0, -1);
        ...
        private void Draw3DBorderRaised(Graphics g, ref Rectangle bounds, ColorData colors)
        {
            ...
            using (WindowsGraphics graphics = WindowsGraphics.FromGraphics(g))
            {
                ...
                Point point3 = new Point(bounds.X, (bounds.Y + bounds.Height) - 1);
                Point point4 = new Point((bounds.X + bounds.Width) - 1, (bounds.Y + bounds.Height) - 1);
                WindowsPen pen = flag ? new WindowsPen(graphics.DeviceContext, SystemColors.ControlLightLight) : new WindowsPen(graphics.DeviceContext, colors.highlight);
                try
                {
                    graphics.DrawLine(pen, point, point2);
                    graphics.DrawLine(pen, point2, point3);
        ...
        protected internal static void Draw3DLiteBorder(Graphics g, Rectangle r, ColorData colors, bool up)
        {
            using (WindowsGraphics graphics = WindowsGraphics.FromGraphics(g))
            {
                ...
                WindowsPen pen = up ? new WindowsPen(graphics.DeviceContext, colors.highlight) : new WindowsPen(graphics.DeviceContext, colors.buttonShadow);
                try
                {
                    graphics.DrawLine(pen, point, point2);
                    graphics.DrawLine(pen, point2, point3);
                ...
                pen = up ? new WindowsPen(graphics.DeviceContext, colors.buttonShadow) : new WindowsPen(graphics.DeviceContext, colors.highlight);
                try
                {
                    point.Offset(0, -1);
                    graphics.DrawLine(pen, point3, point4);
        ...
        internal class ColorOptions
        {
            ...
            internal ButtonBaseAdapter.ColorData Calculate()
            {
                ...
                data.buttonFace = this.backColor;
